...
impl From<&str> for Almanac {
    fn from(input: &str) -> Self {
        ...
        // Seeds is now interpreted as a pairs of numbers
        // leading to ranges of Seed
        ...
        let seeds: Vec<Range<i128>> = seeds
            .chunks(2)
            .map(|values| {
                let start = values[0];
                let span = values[1];
...
fn part1(input: &str) -> i128 {
    ...
    al.seeds
        .iter()
        .filter_map(|range| range.clone().map(|x| al.stream(x)).min())
        .min()
        .expect("msut have at leat one value")
}
